...
# Captura segura da aba ativa selecionada no menu
    ...
    if "Dashboard" in aba_ativa:
        ...
        try:
            ...
            if not df_planos_dash.empty:
                    ...
                    for pref in prefs:
                        ...
                        if crit in ["Horímetro", "Odômetro"]:
                            if not tem_leitura_sistema:
                                saldo_restante, proxima_preventiva_val = 0.0, 0.0
                ...
                if avisos_pendencia_medidor:
                    ...
                    st.warning(f"💡 **Orientação de Medidor:** O(s) veículo(s) **{veiculos_str}** não possuem valores de horímetro/odômetro registrados.")
            ...
            st.error(f"Erro ao carregar o painel preditivo: {e}")
        ...
        c_tit, c_actions = st.columns([0.6, 0.4])
        with c_tit: 
