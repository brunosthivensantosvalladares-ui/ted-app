...
if not st.session_state["logado"]:
    ...
    # --- SINCRONIZAÇÃO ENTRE SIDEBAR E TABS ---
    ...
    # Abas no Topo (Mobile / Navegação Rápida)
    # Criamos as abas e pegamos o índice da aba ativa baseada na sessão
    abas = st.tabs(opcoes)
    aba_ativa_index = opcoes.index(st.session_state.navegacao_ted)

    # Renderização do conteúdo SEMPRE na aba que está no estado da sessão
    with abas[aba_ativa_index]:
        nome_aba = st.session_state.navegacao_ted
        ...
        if nome_aba == "✍️ Abrir Solicitação":
            ...
            with c2: 
                if not df_ind.empty:
                    ...
                    st.caption("🔍 **O que isso mostra?** Mede a eficiência da equipe.")
